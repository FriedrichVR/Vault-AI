...
export default function Scan() {
  ...
  const handleScan = () => {
    ...
    setTimeout(() => {
        ...
        alert('Factura procesada con éxito (Simulación)');
        navigate('/');
    }, 3000);
  };
  ...
  return (
    <div className="fixed inset-0 bg-background-light dark:bg-background-dark z-[100] flex flex-col overflow-hidden">
      ...
      <div className="relative flex-1 bg-neutral-900 overflow-hidden flex items-center justify-center">
        {/* Mock Invoice Image Background */}
        <div className="absolute inset-0 opacity-40">
          <img 
            alt="Documento de factura sobre una mesa de madera oscura" 
            ...
            <div className="flex flex-col gap-3">
              <div className="flex justify-between items-end">
                <div className="flex flex-col gap-1">
                  <p className="text-slate-900 dark:text-slate-100 text-lg font-semibold leading-none">Procesando factura</p>
                  <p className="text-slate-500 dark:text-slate-400 text-xs uppercase tracking-widest font-medium">Extrayendo datos fiscales</p>
                </div>
                <p className="text-primary text-sm font-bold tabular-nums">72%</p>
              </div>
        ...
        {!isProcessing && (
          <div className="flex items-end justify-between px-6 pb-2">
            <div className="flex flex-col items-center gap-2">
              <button className="flex shrink-0 items-center justify-center rounded-full size-12 bg-slate-200 dark:bg-white/5 text-slate-500 dark:text-slate-100 hover:bg-white/10 transition-colors">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[110] p-4">
          <div className="bg-background-light dark:bg-background-dark rounded-xl p-6 w-full max-w-md shadow-2xl border border-white/10">
            ...
            <p className="text-sm text-slate-500 mb-6">Ingresa los detalles de tu factura o gasto manualmente.</p>
            <form className="flex flex-col gap-4" onSubmit={(e) => { e.preventDefault(); setShowModal(false); navigate('/'); }}>
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold uppercase tracking-wider text-slate-500 text-left">Proveedor</label>
                <input type="text" placeholder="Ej. Supermercado" className="px-4 py-3 border border-slate-200 dark:border-white/10 rounded-lg bg-white dark:bg-background-dark text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-primary focus:border-transparent outline-none" required />
              </div>
              <div className="flex flex-col gap-1.5 text-left">
                <label className="text-xs font-semibold uppercase tracking-wider text-slate-500">Fecha</label>
                <input type="date" className="px-4 py-3 border border-slate-200 dark:border-white/10 rounded-lg bg-white dark:bg-background-dark text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-primary focus:border-transparent outline-none" required />
              </div>
              <div className="flex flex-col gap-1.5 text-left">
                <label className="text-xs font-semibold uppercase tracking-wider text-slate-500">Monto</label>
                <input type="number" placeholder="0.00" step="0.01" className="px-4 py-3 border border-slate-200 dark:border-white/10 rounded-lg bg-white dark:bg-background-dark text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-primary focus:border-transparent outline-none" required />
              </div>
              <div className="flex justify-end gap-3 mt-4">
                ...
                <button type="submit" className="px-5 py-2.5 font-medium bg-primary text-white rounded-lg shadow-lg shadow-primary/30">Guardar</button>
              </div>
            </form>
          </div>
        </div>
